...
/** Reads the user file. */
class UserReader(master: ActorRef, userDb: ActorRef) extends Actor with ActorLogging {
  ...
  override def receive: Receive = LoggingReceive {
    case BuildUserDB(fileName) =>
      ...
      if (contents.isEmpty) {
        ...
        val users= UserReader.jsonParse(contents.get)
...
object UserReader  {
  ...
  /** "Constructor" */
  def props(master: ActorRef, userDb: ActorRef) = Props(new UserReader(master, userDb))
